...
public class BookReccomendation {
	public static void main(String[] args) {
		...
		Dataset<Row> movies = spark.read().option("inferschema", true).option("header", true)
					.option("delimiter", ",")
					.csv("/Users/rushikesh/Desktop/spark-aadhar/movie/BX-CSV-Dump/BX-Book-Ratings.csv")
		...
		Dataset<Row> movielink =spark.read().option("inferschema", true)
				//.option("header", true)
				.option("delimiter", ",")
				.csv("/Users/rushikesh/Desktop/spark-aadhar/movie/BX-CSV-Dump/BX-Books.csv")
		...
		.filter(functions.col("title1").contains("Die Hard (1988)")).sort(functions.desc("rc")).show(3000,false);
